...
package com.barryzeha.ktmusicplayer.lyrics
...
class CoverLrcView @JvmOverloads constructor(
    ...
    private fun init(attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, coreRes.styleable.LrcView)
        mCurrentTextSize = ta.getDimension(
            coreRes.styleable.LrcView_lrcTextSize, resources.getDimension(coreRes.dimen.lrc_text_size)
        )
        mNormalTextSize = ta.getDimension(
            coreRes.styleable.LrcView_lrcNormalTextSize,
            resources.getDimension(coreRes.dimen.lrc_text_size)
        )
        if (mNormalTextSize == 0f) {
            mNormalTextSize = mCurrentTextSize
        }
        mDividerHeight = ta.getDimension(
            coreRes.styleable.LrcView_lrcDividerHeight,
            resources.getDimension(coreRes.dimen.lrc_divider_height)
        )
        val defDuration = resources.getInteger(coreRes.integer.lrc_animation_duration)
        mAnimationDuration =
            ta.getInt(coreRes.styleable.LrcView_lrcAnimationDuration, defDuration).toLong()
        mAnimationDuration =
            if (mAnimationDuration < 0) defDuration.toLong() else mAnimationDuration
        mNormalTextColor = ta.getColor(
            coreRes.styleable.LrcView_lrcNormalTextColor,
            ContextCompat.getColor(context, coreRes.color.lrc_normal_text_color)
        )
        ...
        mTimelineTextColor = ta.getColor(
            ...
            ContextCompat.getColor(context, coreRes.color.lrc_timeline_text_color)
        )
        mDefaultLabel = ta.getString(coreRes.styleable.LrcView_lrcLabel)
        mDefaultLabel =
            if (mDefaultLabel.isNullOrEmpty()) context.getString(coreRes.string.empty) else mDefaultLabel
        mLrcPadding = ta.getDimension(coreRes.styleable.LrcView_lrcPadding, 0f)
        mTimelineColor = ta.getColor(
            coreRes.styleable.LrcView_lrcTimelineColor,
            ContextCompat.getColor(context, coreRes.color.lrc_timeline_color)
        )
        val timelineHeight = ta.getDimension(
            coreRes.styleable.LrcView_lrcTimelineHeight,
            resources.getDimension(coreRes.dimen.lrc_timeline_height)
        )
        mPlayDrawable = ta.getDrawable(coreRes.styleable.LrcView_lrcPlayDrawable)
        mPlayDrawable =
            if (mPlayDrawable == null) ContextCompat.getDrawable(
                context,
                coreRes.drawable.ic_play_arrow
            ) else mPlayDrawable
        mTimeTextColor = ta.getColor(
            coreRes.styleable.LrcView_lrcTimeTextColor,
            ContextCompat.getColor(context, coreRes.color.lrc_time_text_color)
        )
        val timeTextSize = ta.getDimension(
            coreRes.styleable.LrcView_lrcTimeTextSize,
            resources.getDimension(coreRes.dimen.lrc_time_text_size)
        )
        mTextGravity = ta.getInteger(coreRes.styleable.LrcView_lrcTextGravity, LrcEntry.GRAVITY_CENTER)
        ta.recycle()
        mDrawableWidth = resources.getDimension(coreRes.dimen.lrc_drawable_width).toInt()
        mTimeTextWidth = resources.getDimension(coreRes.dimen.lrc_time_width).toInt()
